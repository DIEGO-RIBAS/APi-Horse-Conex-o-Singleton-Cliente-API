...
interface
  ...
  TModelTarefa = class
    ...
    public
      ...
      function getTotaltarefasSetDias : TJSONObject;
...
begin
  ...
  begin
    ...
    Result.AddPair('Erro','False');
    Result.AddPair('Total',TJSONNumber.Create(FieldByName('tTarefas').AsString));
  end;
end;

function TModelTarefa.getTotaltarefasSetDias: TJSONObject;
begin
  Result := TJSONObject.Create;
  ...
  begin
    ...
    begin
      Clear;
      Add('select count(*)TotalTarefas from tarefa where DThrLancamento >= Getdate() - 7 and  DThrLancamento <= GetDate() and status = 2');
    end;
    ...
    Result.AddPair('Erro','False');
    Result.AddPair('Total',TJSONNumber.Create(FieldByName('TotalTarefas').AsString));
  end;
end;
