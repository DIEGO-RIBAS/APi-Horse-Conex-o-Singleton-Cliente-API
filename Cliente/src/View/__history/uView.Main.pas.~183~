...
begin
  ...
  begin
    Application.MessageBox('Não há tarefa a ser finalizada.','Atenção !',MB_ICONMASK);
    Exit;
  end;

  if CdsListaTarefasStatus.Value = 2 then
  begin
    Application.MessageBox('Tarefa já finalizada "','Atenção !',MB_ICONMASK);
  end;
...
begin
  ...
  lbTotaltarefasCluidas7Dias.Caption := 'Total de tarefas concluídas nos ultimos 7 dias :' + IntToStr(iTotaltarefas);
end;
